...
void serialize_file(int out_fd, uint32_t* offset, File* file) {
	...
	if (write(out_fd, serialized_header, FILE_HEADER_SIZE) == -1) error("Error writing to file");
	...
	*offset += FILE_CONTENT_OFFSET;
	...
	for (uint32_t i = 0; i < blocks; i++) {
		...
		printf("Block: %s\n", block);
		if (write(out_fd, block, BLOCK_SIZE) == -1) error("Error writing to file");
	}
	...
	if (additional_bytes > 0) {
		...
		if (write(out_fd, block, additional_bytes) == -1) error("Error writing to file");
	}
	free(block);
	block = NULL;
...
int main(int argc, char* argv[]) {
	...
	if (write(fd, base_dir, ENTRY_NAME_SIZE) == -1) error("Error writing to file");
	free(base_dir);
	base_dir = NULL;
	DIR* d;
	struct dirent* dir;
	...
	if (write(fd, &num_files, FILE_COUNT_SIZE) == -1) error("Error writing to file");
	if (close(fd) == -1) error("Error closing file");
	printf("Saved to %s\n", filename);
}
